...
enum PushType {
  ...
  PUSH_TYPE_QIYEWEIXIN = 11  // 企业微信应用
};
...
bool isPushChannelValid(const PushChannel& ch) {
  ...
  switch (ch.type) {
    ...
    case PUSH_TYPE_QIYEWEIXIN:
      return ch.key1.length() > 0 && ch.key2.length() > 0; // 需要发送参数和接收人
    default:
      return false;
  }
}
  ...
  <script>
    ...
    function updateTypeHint(idx) {
      ...
      if (type == 1) {
        ...
        hint.innerHTML = '<b>企业微信应用：</b><br>填写服务器地址（如 https://qyapi.weixin.qq.com）<br>发送参数填写corp_id,agent_id和corp_secret，中间使用","进行分隔';
        extraFields.style.display = 'block';
        document.getElementById('key1label' + idx).innerText = '发送参数';
        document.getElementById('key1' + idx).placeholder = 'corp_id|agent_id|corp_secret';
        // 显示 key2 区域
        document.getElementById('key2' + idx).closest('.form-group').style.display = 'block';
        document.getElementById('key2label' + idx).innerText = '接收人';
        document.getElementById('key2' + idx).placeholder = '@all';
      }
    }
...
void handleRoot() {
  ...
  for (int i = 0; i < MAX_PUSH_CHANNELS; i++) {
    ...
    channelsHtml += "<option value=\"11\"" + String(config.pushChannels[i].type == PUSH_TYPE_QIYEWEIXIN ? " selected" : "") + ">企业微信应用</option>";
    channelsHtml += "</select>";
